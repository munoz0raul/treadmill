...
# The same mc3_18 model, quantized to A16W8 and lowered onto the Hexagon HTP V75
# via QAIRT, can run the inference on the NPU instead of the CPU. It is driven by
# the on-device `qnn-net-run` (one clip per call). Off by default — the published
# app runs on the CPU via onnxruntime for reproducibility. See 2-live-inference/
# bench/ for the CPU-vs-NPU study behind this.
NPU_RUNTIME_DIR = os.path.expanduser("~/models/npu")   # bin/ lib/ dsp/ + .bin
NPU_CONTEXT_BIN = "speed_cnn_a16w8_htpv75.bin"
NPU_INFER_TIMEOUT_S = 5.0    # a single qnn-net-run call is ~0.3s cold; 5s is slack
...
class NpuSpeedBackend:
    """Runs speed_cnn on the Hexagon HTP V75 NPU via the on-device `qnn-net-run`,
    one clip per call. A drop-in alternative to the onnxruntime CPU session used by
    CnnSpeedEstimator: same input tensor (1,3,T,112,112) float32, same scalar
    km/h output.

    Why a subprocess and not onnxruntime's QNN EP? The mc3_18 3D-CNN has 5D Conv3d
    ops that the QNN execution provider cannot place on the HTP (they fall back to
    CPU). The only route to the NPU for this model is the QAIRT context binary
    consumed by `qnn-net-run`. A cold call (process spawn + load the 12 MB context
    + DSP init + infer) is ~0.3 s here; the inference itself is ~26 ms. At ~2 Hz
    (the estimator's cadence) reloading per call is fine — no daemon needed.

    `runtime_dir` must contain (as staged from the QAIRT build server):
        bin/qnn-net-run
        lib/{libQnnHtp.so, libQnnHtpNetRunExtensions.so, libQnnHtpPrepare.so,
             libQnnHtpV75Stub.so, libQnnHtpV75CalculatorStub.so, libQnnSystem.so}
        dsp/{libQnnHtpV75.so, libQnnHtpV75Skel.so, libQnnHtpNetRunExtensions.so}
        speed_cnn_a16w8_htpv75.bin
    """

    def __init__(self, runtime_dir=NPU_RUNTIME_DIR):
        self._dir = os.path.expanduser(runtime_dir)
        self._bin = os.path.join(self._dir, "bin", "qnn-net-run")
        self._backend = os.path.join(self._dir, "lib", "libQnnHtp.so")
        self._ctx = os.path.join(self._dir, NPU_CONTEXT_BIN)
        self._lib = os.path.join(self._dir, "lib")
        self._dsp = os.path.join(self._dir, "dsp")
        self._ok = False

        missing = [p for p in (self._bin, self._backend, self._ctx) if not os.path.exists(p)]
        if missing:
            log.warning("NPU runtime incomplete under %s (missing: %s)",
                        self._dir, ", ".join(os.path.basename(m) for m in missing))
            return

        # Per-call scratch: a fixed input_list pointing at in.raw, rewritten each
        # inference. Persisted so we pay the mkdir/list-write once.
        self._work = os.path.join(os.path.expanduser("~/.cache"), "treadmill_npu")
        os.makedirs(self._work, exist_ok=True)
        self._in_raw = os.path.join(self._work, "in.raw")
        self._out_dir = os.path.join(self._work, "out")
        self._list = os.path.join(self._work, "input_list.txt")
        with open(self._list, "w") as f:
            f.write(f"frames:={self._in_raw}\n")

        # backend_ext.json with a board-absolute config path (the one shipped in
        # the runtime points at the build server). shared_library_path is a bare
        # soname resolved via LD_LIBRARY_PATH.
        self._htp_cfg = os.path.join(self._work, "htp_v75_config.json")
        with open(self._htp_cfg, "w") as f:
            json.dump({"graphs": [{"graph_names": ["speed_cnn_a16w8"],
                                   "vtcm_mb": 0, "O": 3}],
                       "devices": [{"htp_arch": "v75"}]}, f)
        self._backend_ext = os.path.join(self._work, "backend_ext.json")
        with open(self._backend_ext, "w") as f:
            json.dump({"backend_extensions": {
                "shared_library_path": "libQnnHtpNetRunExtensions.so",
                "config_file_path": self._htp_cfg}}, f)

        # Subprocess env: our 2.47 host libs, and ONLY our 2.47 unsigned v75 skel
        # dir on ADSP_LIBRARY_PATH — a firmware fallback dir (board ships 2.46)
        # would win the version race and fail with "Skel lib id mismatch" (1008).
        self._env = dict(os.environ)
        self._env["LD_LIBRARY_PATH"] = self._lib + ":" + self._env.get("LD_LIBRARY_PATH", "")
        self._env["ADSP_LIBRARY_PATH"] = self._dsp

        # Smoke-infer a zero clip to prove the graph actually loads on the HTP.
        try:
            probe = np.zeros((1, 3, CNN_N_FRAMES, CNN_CLIP, CNN_CLIP), dtype=np.float32)
            self.infer(probe)
            self._ok = True
            log.info("NPU backend ready (Hexagon HTP V75) from %s", self._dir)
        except Exception as e:
            log.warning("NPU smoke inference failed (%s) — NPU backend unavailable", e)
            self._ok = False
    ...
    def infer(self, clip) -> float:
        """Run one (1,3,T,112,112) float32 clip on the NPU and return km/h."""
        clip.astype(np.float32).tofile(self._in_raw)
        subprocess.run(
            [self._bin,
             "--backend", self._backend,
             "--retrieve_context", self._ctx,
             "--input_list", self._list,
             "--config_file", self._backend_ext,
             "--output_dir", self._out_dir],
            env=self._env, cwd=self._work, timeout=NPU_INFER_TIMEOUT_S,
            stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, check=True)
        out = os.path.join(self._out_dir, "Result_0", "speed_kmh.raw")
        return float(np.fromfile(out, dtype=np.float32).flat[0])
...
class CnnSpeedEstimator:
    ...
    def __init__(self, model_path=CNN_MODEL, n_frames=CNN_N_FRAMES,
                 ...
                 provider="cpu", npu_runtime=NPU_RUNTIME_DIR, npu_strict=False):
        self._n = n_frames
        self._clip = clip
        self._smooth = smooth
        self._stride = stride
        ...
        # ── NPU backend (optional). If it comes up we run inference there and skip
        # loading the CPU onnxruntime session entirely. If it fails to initialise,
        # fall back to the CPU — unless --npu-strict, in which case we stay down so
        # the failure is loud (used when the video MUST prove the NPU is in play).
        if provider == "qnn":
            npu = NpuSpeedBackend(npu_runtime)
            if npu.ready:
                self._npu = npu
                self._provider = "qnn"
                log.info("CNN estimator: provider=qnn (Hexagon HTP V75) — clip span %.2fs",
                         self._span)
                self._running = True
                ...
                return
            if npu_strict:
                log.error("NPU backend unavailable and --npu-strict set — CNN speed "
                          "estimation disabled (no CPU fallback)")
                return
            log.warning("NPU backend unavailable — falling back to CPU (onnxruntime)")
        ...
        self._provider = "cpu"
        log.info("CNN estimator: provider=cpu (onnxruntime) — loaded from %s "
                 "(clip span %.2fs, time-based)", model_path, self._span)
        self._running = True
        self._worker = threading.Thread(target=self._infer_loop, daemon=True)
        self._worker.start()
    ...
    @property
    def ready(self) -> bool:
        return self._sess is not None or self._npu is not None

    @property
    def provider(self) -> str:
        return self._provider

    def _infer(self, clip) -> float:
        """Run one clip on the active engine (CPU onnxruntime or the NPU) and
        return the raw km/h scalar before smoothing/calibration."""
        if self._npu is not None:
            return self._npu.infer(clip)
        return float(self._sess.run(["speed_kmh"], {"frames": clip})[0].flat[0])
    ...
    def _infer_loop(self):
        while self._running:
            ...
            if mean_mov < 0.006:
                ...
                continue
            try:
                raw = self._infer(clip)
            except Exception as e:
                # A single dropped inference (subprocess timeout/error on the NPU,
                # or an onnxruntime hiccup) must not kill the loop — skip the clip
                # and keep the last smoothed speed, like a dropped frame.
                log.warning("inference failed on this clip (%s) — skipping", e)
                time.sleep(0.05)
                continue
...
def main():
    ...
    parser.add_argument("--provider", choices=("cpu", "qnn"), default="cpu",
                        help="Inference engine: cpu = onnxruntime (default, the "
                             "published app); qnn = the Hexagon HTP V75 NPU via "
                             "qnn-net-run (needs --npu-runtime staged on the board)")
    parser.add_argument("--npu-runtime", default=NPU_RUNTIME_DIR,
                        help="NPU runtime dir with bin/ lib/ dsp/ + the context "
                             ".bin (default: %(default)s). Only used with "
                             "--provider qnn.")
    parser.add_argument("--npu-strict", action="store_true",
                        help="With --provider qnn, do NOT fall back to the CPU if "
                             "the NPU fails to come up — fail loudly instead (use "
                             "when the run must be proven to be on the NPU).")
    ...
    if args.manual:
        ...
        _cnn = CnnSpeedEstimator(model_path=args.cnn_model,
                                 ...
                                 npu_runtime=args.npu_runtime,
                                 npu_strict=args.npu_strict)
        if not _cnn.ready:
